...
class OsWorkPeriodsHelper {
				...
				if ( $filter->exact_match ) {
					...
					$query_args['AND'][] = [
						'OR' => [
							'custom_date' => 'IS NULL',
							'AND'         => [
								'custom_date >=' => $filter->date_from,
								'custom_date <=' => $filter->date_to,
							],
						],
					];
				}
   ...
   * Returns array in format [start_minutes, end_minutes], for example 08:00-18:00 will be returned as [480, 1080]
	...
	public static function get_work_start_end_time_for_date_multi_agent( array $agent_ids, \LatePoint\Misc\Filter $filter ): array {
		...
		if ( empty( $work_start_times ) ) {
			$work_start_times = [ 0 ];
		}
		if ( empty( $work_end_times ) ) {
			$work_end_times = [ 0 ];
		}
	...
	public static function save_work_periods( $work_periods_to_save, $force_new = false ) {
		...
		if ( ! $force_new ) {
			...
			$search_args = ( isset( $work_period_obj ) ) ? array(
				'agent_id'    => $work_period_obj->agent_id,
				'service_id'  => $work_period_obj->service_id,
				'location_id' => $work_period_obj->location_id,
			) : array();
	...
	public static function get_periods_ids_by_args( $args = array() ) {
		...
		$args         = array_merge( $default_args, $args );
		if ( $args['custom_date'] ) {
			$query_args['custom_date'] = $args['custom_date'];
		}
		if ( $args['week_day'] ) {
	...
	public static function set_default_working_hours() {
		...
		if ( ! self::$existing_work_periods ) {
			self::$existing_work_periods = $work_periods_model->select( 'week_day' )->where(
				array(
					'agent_id'    => 0,
					'service_id'  => 0,
					'location_id' => 0,
				) 
			)->where( array( 'custom_date' => 'IS NULL' ) )->group_by( 'week_day' )->get_results( ARRAY_A );
			if ( self::$existing_work_periods ) {
				self::$existing_work_periods = array_map(
					function ( $work_period ) {
	...
	public static function generate_days_with_custom_schedule( $args = array() ) {
		...
		)->group_by( 'custom_date, chain_id' )->order_by( 'custom_date asc' )->get_results_as_models();
		$html         = '';
		if ( $work_periods && isset( $work_periods[0] ) ) {
			$date            = new OsWpDateTime( $work_periods[0]->custom_date );
			$processing_year = $date->format( 'Y' );
			if ( $date->format( 'Y' ) != gmdate( 'Y' ) ) {
				$html .= '<div class="os-form-sub-header sub-level"><h3>' . esc_html( $date->format( 'Y' ) ) . '</h3></div>';
			}
		}
		$chained_periods = [];
		...
		if ( $work_periods ) {
			...
			foreach ( $work_periods as $work_period ) {
				$i = $i + 1;
				if ( empty( $work_period->custom_date ) ) {
					continue;
				}
				...
				if ( $chained_periods ) {
					...
					foreach ( $chained_periods as $chain_id => $chained_period ) {
						...
						$work_periods_for_date       = $work_periods_for_date_model->where( $args )->where(
							[
								'custom_date' => $range_start_date->format( 'Y-m-d' ),
								'chain_id'    => $chain_id,
							] 
				...
				if ( empty( $work_period->chain_id ) ) {
					...
					if ( $processing_year != $date->format( 'Y' ) ) {
						$html .= '</div><div class="os-form-sub-header sub-level"><h3>' . esc_html( $date->format( 'Y' ) ) . '</h3></div><div class="custom-day-work-periods">';
					}
	...
	public static function generate_custom_day_period_action( $target_date = false, $day_off = false, $args = array() ) {
		...
		if ( ! empty( $os_params ) ) {
			$html .= ' data-os-params="' . esc_attr( OsUtilHelper::build_os_params( $os_params ) ) . '"';
		}
		return $html;
	}
