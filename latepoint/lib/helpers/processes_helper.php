...
class OsProcessesHelper {
	...
	public static function check_if_process_exists( OsProcessModel $process ): bool {
		...
		$existing_process = $existing_process->select( 'id' )->where(
			[
				...
				if ( $group['type'] == 'group' && ! empty( $group['trigger_condition'] ) ) {
					...
					foreach ( $group['items'] as $item ) {
						...
						if ( $item['items'] ) {
							...
							if ( ! empty( $temp ) ) {
								$trigger_conditions = array_merge( $trigger_conditions, $temp );
							}
						}
					}
				}
	...
	public static function trigger_conditions_html_for_event( \LatePoint\Misc\ProcessEvent $event ): string {
		if ( empty( \LatePoint\Misc\ProcessEvent::get_available_trigger_condition_objects_for_event_type( $event->type ) ) ) {
			return '';
		}
		...
		if ( $groups ) {
			foreach ( $groups as $group ) {
				if ( $group['type'] == 'action' ) {
					...
					$actions[]   = new \LatePoint\Misc\ProcessAction(
						[
							'type'     => $action_data['type'],
							'id'       => $group['id'],
							'status'   => $action_data['status'] ?? LATEPOINT_STATUS_ACTIVE,
