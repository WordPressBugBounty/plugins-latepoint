...
class OsEventsHelper {
	...
	public static function week_grid( OsWpDateTime $calendar_start, OsWpDateTime $calendar_end, OsWpDateTime $target_date, array $services_with_resources, array $filter = [] ) {
		...
		for ( $day_date = clone $calendar_start; $day_date <= $calendar_end; $day_date->modify( '+1 day' ) ) {
			...
			foreach ( $services_with_resources as $service_resources ) {
				if ( empty( $service_resources['resources'][ $day_date->format( 'Y-m-d' ) ] ) ) {
					continue;
				}
				$clean_periods = [];
				foreach ( $service_resources['resources'][ $day_date->format( 'Y-m-d' ) ] as $resource ) {
					if ( ! empty( $filter['agent_id'] ) && $resource->agent_id != $filter['agent_id'] ) {
						continue;
					}
					if ( ! empty( $filter['service_id'] ) && $resource->service_id != $filter['service_id'] ) {
						continue;
					}
					if ( ! empty( $filter['location_id'] ) && $resource->location_id != $filter['location_id'] ) {
						continue;
					}
		...
		for ( $day_date = clone $calendar_start; $day_date <= $calendar_end; $day_date->modify( '+1 day' ) ) {
			...
			if ( $day_date->format( 'd' ) == '1' ) {
				$html .= '<div class="os-day-month">' . OsUtilHelper::get_month_name_by_number( $day_date->format( 'n' ) ) . '</div>';
			}
			$html .= '<div class="le-day-number" 
					data-os-params="' . OsUtilHelper::build_os_params(
				[
					'target_date_string' => $day_date->format( 'Y-m-d' ),
					'filter'             => $filter,
				] 
			) . '" 
	...
	public static function event_service_box_html( OsServiceModel $service, \LatePoint\Misc\TimePeriod $time_period, array $filter, OsWpDateTime $start_date, string $extra_attrs = '' ) {
		...
		if ( ! empty( $filter['agent_id'] ) ) {
			$selected_attributes .= ' data-selected-agent="' . esc_attr( $filter['agent_id'] ) . '" ';
		}
		if ( ! empty( $filter['location_id'] ) ) {
			$selected_attributes .= ' data-selected-location="' . esc_attr( $filter['location_id'] ) . '" ';
		}
		if ( ! empty( $filter['show_agents'] ) ) {
			$selected_attributes .= ' data-show-agents="' . esc_attr( $filter['show_agents'] ) . '" ';
		}
		if ( ! empty( $filter['show_locations'] ) ) {
			$selected_attributes .= ' data-show-locations="' . esc_attr( $filter['show_locations'] ) . '" ';
		}
	...
	public static function events_grid( OsWpDateTime $target_date, array $filter = [], $range_type = 'week', array $restrictions = [] ): string {
		...
		switch ( $range_type ) {
			case 'week':
				...
				if ( $shift ) {
					$calendar_start->modify( '-' . $shift . ' days' );
				}
		...
		if ( ! empty( $restrictions['show_services'] ) ) {
			$services->where( [ 'id' => explode( ',', $restrictions['show_services'] ) ] );
		}
		...
		foreach ( $services as $service ) {
			$services_with_resources[ 'service_' . $service->id ] = [
				'model'        => $service,
				'work_periods' => [],
			];
		...
		foreach ( $filter as $filter_key => $filter_value ) {
			if ( ! empty( $filter_value ) ) {
				$clean_filter[ $filter_key ] = $filter_value;
			}
		}
		...
		if ( $range_type == 'month' ) {
			...
			$html     .= '<div class="le-range-selector">' . OsFormHelper::select_field(
				'calendar_range_type',
				false,
				[
					'week'  => __( 'Weekly', 'latepoint' ),
					'month' => __( 'Monthly', 'latepoint' ),
				],
				$range_type 
			) . '</div>';
				...
				$html .= OsFormHelper::select_field(
					...
					array_merge(
						[
							[
								'value' => '',
								'label' => __( 'All Agents', 'latepoint' ),
							],
						],
						OsAgentHelper::get_agents_list( false, $agent_restriction ) 
					),
					$clean_filter['agent_id'] ?? '' 
				);
				$html .= OsFormHelper::select_field(
					'filter[service_id]',
					false,
					array_merge(
						[
							[
								'value' => '',
								'label' => __( 'All Services', 'latepoint' ),
							],
						],
						OsServiceHelper::get_services_list( false, $service_restriction ) 
					),
					$clean_filter['service_id'] ?? '' 
				);
			$html     .= '</div>';
		$html         .= '</div>';
