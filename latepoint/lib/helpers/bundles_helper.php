...
class OsBundlesHelper {
	...
	public static function generate_order_summary_for_bundle( OsBundleModel $bundle, string $order_item_id, $preselected_booking_id = false ): string {
		...
		foreach ( $bundle_services as $service ) {
			...
			if ( count( $bundle_services ) > 1 ) {
				$html .= '<div class="bundle-service-info">' . $service->name . ' [' . sprintf( __( '%d sessions', 'latepoint' ), $service->join_attributes['quantity'] ) . ']</div>';
			}
	...
	public static function generate_summary_for_bundle( OsBundleModel $bundle, $cart_item_id = false, $order_item_id = false, $user_type = LATEPOINT_USER_TYPE_ADMIN ) {
		ob_start();
		?>
		<div class="summary-box main-box" <?php if ( $cart_item_id ) {
			echo 'data-cart-item-id="' . esc_attr( $cart_item_id ) . '"';} ?>>
			<?php
			$bundle_headings = [];
			...
			?>
			<div class="summary-box-content <?php if ( $cart_item_id ) {
				echo 'os-cart-item';} ?> is-removable">
			...
			<?php if ( $order_item_id ) {
				...
				foreach ( $bundle_services as $service ) {
					...
					// translators: %d is the number of sessions
					if ( count( $bundle_services ) > 1 ) {
						echo '<div class="bundle-service-info">' . esc_html( $service->name . ' [' . sprintf( __( '%d sessions', 'latepoint' ), $service->join_attributes['quantity'] ) . ']' ) . '</div>';
					}
					for ( $i = 0; $i < $service->join_attributes['quantity']; $i++ ) {
						if ( isset( $bookings[ $i ] ) ) {
							...
							if ( $user_type == LATEPOINT_USER_TYPE_CUSTOMER ) {
								...
								echo '<div class="order-item-variant-bundle-booking" ' . OsOrdersHelper::quick_order_btn_html(
									false,
									[
