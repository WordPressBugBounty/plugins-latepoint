...
class OsAgentModel extends OsModel {
	...
	protected function set_defaults() {
		if ( empty( $this->status ) ) {
			$this->status = LATEPOINT_AGENT_STATUS_ACTIVE;
		}
	}
	...
	protected function get_total_synced_future_bookings() {
		$bookings = new OsBookingModel();
		$total    = $bookings->where(
			[
				'agent_id'                                 => $this->id,
				LATEPOINT_TABLE_BOOKING_META . '.meta_key' => 'google_calendar_event_id',
			] 
		)
		...
		if ( ! empty( $this->features ) ) {
			...
			if ( ! empty( $features ) ) {
				foreach ( $features as $feature ) {
					if ( $feature['value'] && $feature['label'] ) {
	...
	public function save_services() {
		foreach ( $this->services as $service ) {
			...
			if ( $service_connection_row ) {
				$update_data = array(
					'is_custom_hours'    => $service->is_custom_hours,
					'is_custom_price'    => $service->is_custom_price,
					'is_custom_duration' => $service->is_custom_duration,
				);
